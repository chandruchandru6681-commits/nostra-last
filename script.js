...
const productCategoryMap = {
  'varsi leather bag': ['best-seller'],
  'fit twill shirt for woman': ['hot-collection', 'new-arrival'],
  'grand atlantic chukka boots': ['trendy'],
  "women's faux-trim shirt": ['best-seller'],
  'soft touch interlock polo': ['trendy'],
  'casmart smart watch': ['hot-collection'],
  'casmart smart glass': ['best-seller'],
  'cotton shirt for men': ['hot-collection'],
  'double-breasted blazer': ['trendy'],
  'ribbed cotton bodysuits': ['new-arrival']
};
...
const matchesCategory = (item, category) => {
  ...
  const categories = productCategoryMap[title] || [];
...
const matchesSearchQuery = (item, query) => {
  ...
  return title.includes(query) || imageAlt.includes(query);
};
...
const filterProducts = (category = 'all', query = '') => {
  ...
  productItems.forEach((item) => {
    const showByCategory = matchesCategory(item, category);
    const showBySearch = matchesSearchQuery(item, normalizedQuery);
    item.style.display = showByCategory && showBySearch ? '' : 'none';
  });
};

const updateActiveFilter = (clickedButton) => {
  filterButtons.forEach((button) => button.classList.toggle('active', button === clickedButton));
};

const getActiveFilter = () => {
  const activeButton = document.querySelector('.filter-list button.active');
  return activeButton ? activeButton.dataset.filter || 'all' : 'all';
};

const handleFilterClick = (event) => {
  const clickedButton = event.currentTarget;
  const filterCategory = clickedButton.dataset.filter || 'all';
  const query = searchInput?.value || '';

  updateActiveFilter(clickedButton);
  filterProducts(filterCategory, query);
};

const handleSearch = () => {
  const query = searchInput?.value || '';
  const filterCategory = getActiveFilter();
  filterProducts(filterCategory, query);
};

const initSearchFilter = () => {
  if (!searchInput || !searchButton || !productItems.length) return;
...
wishlist = wishlist.map(name => name.trim());
...
const updateWishlistIcons = () => {
  document.querySelectorAll('.product-card').forEach(card => {
    ...
    if (titleEl) {
      ...
      if (heartIcon) {
        const isLiked = wishlist.includes(productName);
  ...
  const wishlistBadges = document.querySelectorAll('.btn-badge:not(.green)');
...
const toggleWishlist = (productName) => {
  ...
  if (index > -1) {
    ...
    wishlist.push(trimmedName);
    showToast(`${trimmedName} added to wishlist!`);
  }
  localStorage.setItem('wishlist', JSON.stringify(wishlist));
  updateWishlistIcons();
};

// Handle heart clicks using event delegation
document.addEventListener('click', function(e) {
  const heartBtn = e.target.closest('button[aria-label="Add to Whishlist"]');
  if (heartBtn) {
...
// Initialize wishlist icons on load
setTimeout(() => {
  updateWishlistIcons();
}, 100);
...
const updateCartBadge = () => {
  ...
  const cartBadges = document.querySelectorAll('.btn-badge.green');
...
const showToast = (message) => {
  // Create a simple toast if notification system exists
  if (notificationToast) {
    const toastDetail = notificationToast.querySelector('.toast-detail');
    if (toastDetail) {
      toastDetail.querySelector('.toast-message').textContent = message;
...
document.addEventListener('click', function(e) {
  ...
  if (cartBtn) {
    ...
    const titleEl = card.querySelector('.card-title a');
    const priceEl = card.querySelector('.card-price data');
    const imgEl = card.querySelector('.card-banner img');

    if (titleEl && priceEl && imgEl) {
      const product = {
        name: titleEl.textContent.trim(),
        price: parseInt(priceEl.getAttribute('value')),
        image: imgEl.src
      };
      console.log('Adding to cart:', product);
      addToCart(product);
    } else {
      console.error('Could not find product elements:', { titleEl, priceEl, imgEl });
    }
  }
});
